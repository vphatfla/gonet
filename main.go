...
func (s *scanner) scanPort(eth *layers.Ethernet, ip4 *layers.IPv4, tcp *layers.TCP) (string, time.Duration, error) {
    // Start sending packet
    start := time.Now()

    if err := s.send(eth, ip4, tcp); err != nil {
        return "", 0, fmt.Errorf("Error sending to port %v", tcp.DstPort)
    }
    ...
    if ipLayer == nil {
        return "", d, fmt.Errorf("No IP layer")
    }

    tcpLayer := packet.Layer(layers.LayerTypeTCP)
    if tcpLayer == nil {
        return "", d, fmt.Errorf("No TCP layer")
    }

    tcpSegment, _ := tcpLayer.(*layers.TCP)
    if tcpSegment.RST {
        return fmt.Sprintf("Port %v closed (RST)", tcp.DstPort), d, nil
    } else if tcpSegment.SYN && tcpSegment.ACK {
       return fmt.Sprintf("Port %v open (SYN & ACK)", tcp.DstPort), d, nil
    } else {
        return fmt.Sprintf("Port %v might be blocked (filtered)", tcp.DstPort), d, nil
    }
}
func main() {
    ...
    hwaddr, err := s.getHwAddr()
    if err != nil {
        log.Fatalf("Error getting hwaddr dest %v", err)
    }
    eth := layers.Ethernet {
        SrcMAC: s.iface.HardwareAddr,
        DstMAC: hwaddr,
    ...
    res, d, err :=  s.scanPort(&eth, &ip4, &tcp)
    if err != nil {
        log.Fatal(err)
    }
